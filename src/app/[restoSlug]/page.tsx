...
const OpeningHours: Record<number, string[]> = {
...
const menuDataRestaurantStafa: MenuData = [
  ...
  {
    section: "Fisch und Crevetten",
    description: "WIE FERIEN AM MEER",
    items: [
  ...
  },
  {
    ...
    description: "MALAYSISCHE SPEZIALITÄTEN UND SCHWEIZER KLASSIKER",
    items: [
  ...
  {
    section: "Poulet",
    description: "ASIATISCH INTERPRETIERT",
    items: [
  ...
  {
    section: "Rind",
    description: "KLASSISCH UND EXOTISCH",
    items: [
  ...
  {
    section: "Lamm",
    description: "HARMONISCH KOMBINIERT",
    items: [
  ...
  {
    section: "Vegi",
    description: "ÜBERRASCHEND UND GESUND",
    items: [
  ...
  {
    section: "Beilagen",
    description: "LECKERE NEBENSACHE",
    items: [
...
const menuDataBistroSchiffsteg: MenuData = [
  ...
  {
    section: "Warme und kalte Gerichte",
    description: "FÜR JEDEN GESCHMACK",
    items: [
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-screen relative flex flex-col gap-8 pt-35">
        ...
        >
          <Image src={restaurant.image} alt="Chef" fill className="object-cover brightness-35 rounded-3xl p-3" sizes="100vw" />
        </motion.div>

        <div className="w-6xl h-[calc(100vh-12rem)] mx-auto flex justify-between gap-4 z-10 pt-20 relative">
          <motion.div
            className="flex flex-col text-white gap-4 font-(family-name:--font-lora)"
            initial={{ opacity: 0, x: -24 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ ...restoTween, delay: 0.15 }}
          >
            <h2 className="text-6xl font-bold">{restaurant.title}</h2>
            <motion.h3
          ...
          <motion.div
            className="flex flex-col items-start h-fit min-w-60 p-2 rounded-md gap-2 z-10 bg-white/5 backdrop-blur-xs"
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ ...restoTween, delay: 0.3 }}
          >
            <div className="text-md font-normal text-white">Öffnungszeiten</div>
            <div className="flex flex-col gap-1 w-full bg-white/10 p-2 rounded-md backdrop-blur-lg">
              {OpeningHours[safeRestoId]?.map((item: string, index: number) => (
                <div key={index} className="text-md text-white">
                  {item}
                </div>
              ))}
            </div>
          ...
          <motion.div
            className="w-fit absolute bottom-0 right-0 flex flex-col gap-1 border border-white/5 p-2 rounded-md backdrop-blur-xs"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ ...restoTween, delay: 0.4 }}
          >
            <h2 className="text-md font-normal text-white">Our Restaurants</h2>
            <div className="flex gap-2">
              {RESTAURANTS.filter((item) => item.id !== safeRestoId).map((item, i) => (
                ...
                >
                  <Link href={`/${item.slug}`}>
                    <motion.div
                      className="flex w-full items-center gap-2 bg-white/10 p-2 rounded-md backdrop-blur-lg hover:scale-102 transition-all duration-300"
      ...
      <div className="w-6xl h-fit flex gap-8 pt-5">
        <div className="w-[75%] relative flex flex-col gap-2 p-5">
          <motion.h1
            className="text-5xl font-bold text-(--salz-color)"
            initial={{ opacity: 0, y: 16 }}
                  ...
                  {sect.items.map((item, index) => (
                    ...
                    >
                      ...
                      <div className="w-70 pt-2 overflow-hidden text-black flex flex-col gap-1 p-2 bg-white/30 group-hover:bg-white/80 transition-all duration-300 backdrop-blur-xs">
                        ...
                        {item.description && <h4 className="w-60 text-xs text-normal text-gray-600">{item.description}</h4>}
                      </div>
                        ...
                        {"priceVariants" in item ? (
                          <div className="flex flex-col items-end gap-0.5">
                            {Object.entries(item.priceVariants).map(([variant, priceVal]) => (
                              <h4 key={variant} className="text-sm text-normal whitespace-nowrap text-ellipsis overflow-hidden">
                                {variant} {priceVal}
                              </h4>
                            ))}
                          </div>
        ...
        <motion.div
          className="w-[25%] sticky top-28 self-start flex flex-col gap-4 "
          initial={{ opacity: 0, x: 24 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ ...restoTween, delay: 0.15 }}
        >
          <div className="p-4 main-bg border border-gray-200 shadow rounded-md">
            <div className="text-black text-sm">Take-Away nach Vorbestellung</div>
          </div>
          ...
          <motion.div
            className="w-full gap-2 z-10 rounded-md overflow-hidden bg-muted relative"
            initial={{ opacity: 0, y: 12 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
